...
public class KhoseRecordProcessor implements IRecordProcessor {
    ...
    private void processSingleRecord(final Record record) {
        try {
            ...
            LOG.error("Malformed data: " + stringifyRecord(record), e);
        } catch (IOException e) {
            LOG.error("Unhandled exception: " + stringifyRecord(record), e);
        }
    }

    private String stringifyRecord(final Record record) {
        try {
